...
void main()
{
	...
	vec4 sampleColor = texture2D(DiffuseTexture, fTexCoord);
	
	if (false) //Simple blur
	{
		sampleColor = sampleColor * 4;
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2(-1,-2)*pixelSize) * 1;
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2( 0,-2)*pixelSize) * 2;
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2( 1,-2)*pixelSize) * 1;
		
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2(-2,-1)*pixelSize) * 1;
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2(-1,-1)*pixelSize) * 2;
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2( 0,-1)*pixelSize) * 4;
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2( 1,-1)*pixelSize) * 2;
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2( 2,-1)*pixelSize) * 1;
		
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2(-2, 0)*pixelSize) * 2;
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2(-1, 0)*pixelSize) * 4;
		// sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2( 0, 0)*pixelSize) * 8;
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2( 1, 0)*pixelSize) * 4;
		sampleColor = sampleColor + texture2D(DiffuseTexture, fTexCoord + vec2( 2, 0)*pixelSize) * 2;
